...
import { Menu, X, BarChart3, Users, TrendingUp, FileText, Download, Settings, LogOut } from 'lucide-react';

/**
 * College Portal - LOCKED DESIGN SYSTEM
 * Deep Space Navy + Sky Blue + Glassmorphism (matches the Hero Section)
 * For college administrators to manage students and view analytics
 */
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          {sidebarOpen && (
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg text-foreground">College</span>
            </div>
          )}
          <motion.button
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[
          { label: 'Enrolled Students', value: '1,234' },
          { label: 'Placed Students', value: '1,087' },
      ...
      <div className="glass rounded-2xl p-8">
        ...
        <p className="text-foreground/70">Your college activity will appear here.</p>
      </div>
    </motion.div>
  );
}
...
function DownloadsTab() {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
      ...
      <div className="glass rounded-2xl p-8">
        <p className="text-foreground/70">Downloads interface coming soon.</p>
      </div>
    </motion.div>
  );
}
